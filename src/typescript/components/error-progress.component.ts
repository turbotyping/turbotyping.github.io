...
const PROGRESS_CANVAS_ID = 'error-progress-canvas-id';
const NOT_ENOUGH_SAMPLES_ID = 'not-enough-error-samples-id';
const GRID_LINES_COLOR_IN_DARK_THEME = '#333';
const GRID_LINES_COLOR_IN_LIGHT_THEME = '#eeeeee';

export class ErrorProgressHtmlComponent extends BaseHtmlComponent {
  ...
  _toHtml() {
    return /* html */ `
      <div>
        <p id="${NOT_ENOUGH_SAMPLES_ID}" class="not-enough-samples">Not enough samples to display error progress</p> 
    ...
    if (!stats || stats.length < MIN_STATS_TO_DISPLAY) {
      ...
      new Chart(this.canvas.getContext('2d'), {
        ...
        data: {
          ...
          datasets: [
            {
              data: stats.map((stat) => stat.errors),
              label: 'Errors',
              borderColor: '#e06c75',
              fill: false,
            },
